...
//chy add @20120312 for support multi rild client
...
#if MULTI_CLIENT
	int fd; 			//client fd, used for response to the corresponding socket
#endif
} RequestInfo;
...
//atic int s_fdCommand = -1;
static int s_fdDebug = -1;

static int s_fdWakeupRead;
static int s_fdWakeupWrite;

//atic struct ril_event s_commands_event;
...
#if MULTI_CLIENT
#define MEDIA_PROCESS "media"  
#define RILD_CLINET_NUM 2
static int s_clientFd[RILD_CLINET_NUM] = {-1,-1};
static struct ril_event s_commands_event[RILD_CLINET_NUM];
#else
...
#if MULTI_CLIENT
static int getCorrespondingEventIndex(int fd);
static int processCommandBuffer(int fd, void *buffer, size_t buflen);
static int sendResponseRaw (int fd, const void *data, size_t dataSize);
static int sendResponse (int fd, Parcel &p);
...
static void processCommandsCallback(int fd, short flags, void *param){
    ...
    assert((fdCommand == s_clientFd[0]) || (fdCommand == s_clientFd[1]));
    ...
    for (;;) {
        ...
        if (ret == 0 && p_record == NULL) {
			...
			processCommandBuffer(fdCommand, p_record, recordlen);
        }
    }
    ...
    if (ret == 0 || !(errno == EAGAIN || errno == EINTR)) {
        ...
        index = getCorrespondingEventIndex(fdCommand);
		close(fdCommand);
		ril_event_del(&s_commands_event[index]);
		s_clientFd[index] = -1;
...
static void listenCallback (int fd, short flags, void *param){
	...
	assert ((s_clientFd[0]< 0) || (s_clientFd[1]<0));
    assert (fd == s_fdListen);

    fdCommand = accept(s_fdListen, (sockaddr *) &peeraddr, &socklen);
    ...
    if (err == 0 && szCreds > 0) {
        ...
        if (pwd != NULL) {
            if ((strcmp(pwd->pw_name, PHONE_PROCESS) == 0) || (strcmp(pwd->pw_name, MEDIA_PROCESS) == 0)) {
                is_phone_socket = 1;
            } else {
    ...
    p_rs = record_stream_new(fdCommand, MAX_COMMAND_BYTES);
	//chy add, Find a untapped event to set value.
	index = getCorrespondingEventIndex(-1);
	s_clientFd[index] = fdCommand;
	ril_event_set (&s_commands_event[index], fdCommand, 1,
		processCommandsCallback, p_rs);
	rilEventAddWakeup (&s_commands_event[index]);
	onNewCommandConnect(fdCommand);
}
...
static int sendResponse (int fd, Parcel &p){
	printResponse;
	return sendResponseRaw(fd, p.data(), p.dataSize());
}
static void onNewCommandConnect(int fd){
    ...
    if (s_lastNITZTimeData != NULL) {
        sendResponseRaw(fd, s_lastNITZTimeData, s_lastNITZTimeDataSize);
...
static int getCorrespondingEventIndex(int fd){
	int i = 0;
	for(i=0; i<RILD_CLINET_NUM; i++){
		if(s_commands_event[i].fd == fd){
			LOGE("%s(),index=%d", __FUNCTION__, i);
			return i;
		}
	}
	LOGE("%s(),index=%d", __FUNCTION__, i);
...
static void debugCallback (int fd, short flags, void *param) {
    ...
    switch (atoi(args[0])) {
        ...
        case 1:
            ...
            // Close the socket
#if MULTI_CLIENT	
			for(int i=0; i<RILD_CLINET_NUM; i++){
				if(s_clientFd[i] > 0){
					close(s_clientFd[i]);
					s_clientFd[i] = -1;
				}
...
eventLoop(void *param) {
    ...
    ril_event_init();
#if MULTI_CLIENT
	//chy add to init s_commands_event[]	
	memset(&s_commands_event, 0, RILD_CLINET_NUM*sizeof(struct ril_event));
	for(int i=0; i<RILD_CLINET_NUM; i++)
		s_commands_event[i].fd = -1;
#endif
...
#if MULTI_CLIENT
	/* note: persistent so we can accept multiple connection  */
                                ...
                                size_t datalen)
{
	...
	for(int i=0; i < RILD_CLINET_NUM; i++){
		if(s_clientFd[i] == -1)
			continue;
		ret = sendResponse(s_clientFd[i], p);
...
#else
    ret = sendResponse(s_clientFd[0], p);
    if (ret != 0 && unsolResponse == RIL_UNSOL_NITZ_TIME_RECEIVED) {
